...
export async function createAnkiApkg(
  ...
  for (const card of cards) {
    ...
    sourceToTargetDeck.addCard(card.front, backWithAudio)
  }
  ...
  // If reversed cards are enabled, create Target -> Source deck
  if (includeReversedCards) {
    ...
    for (const card of cards) {
      ...
      const backWithAudio = `${card.back}<br>[sound:${audioFileName}]`
      targetToSourceDeck.addCard(backWithAudio, card.front)
    }
